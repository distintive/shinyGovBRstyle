...
text_Input <- function(inputId, label, hint_label=NULL, type = "text",
  ...
  if (is.null(width)){
    ...
    width_class <- paste0("govbr-input govbr-input--width-", width)
  }
  govText <- shiny::tags$div(class="govbr-form-group", id=paste0(inputId,"div"),
    ...
    if (is.null(prefix) & is.null(suffix))
      ...
      shiny::tags$div(class="govbr-input__wrapper",
      shiny::tags$div(
        prefix, class="govbr-input__prefix", `aria-hidden`="true"
      ),
      shiny::tags$input(id=inputId, class=width_class, type = type))}
      ...
      shiny::tags$div(class="govbr-input__wrapper",
               shiny::tags$input(id=inputId, class=width_class, type = type),
               shiny::tags$div(
                 suffix, class="govbr-input__suffix", `aria-hidden`="true"
               )
      )}
    else {
      shiny::tags$div(class="govbr-input__wrapper",
